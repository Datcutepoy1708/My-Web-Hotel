<?php
// Lấy message từ session nếu có (từ ProfileController hoặc xử lý trước đó)
...
$processedByController = isset($_SESSION['processed_by_controller']) && $_SESSION['processed_by_controller'];
unset($_SESSION['processed_by_controller']);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_profile']) && !$processedByController) {
    ...
    if (isset($_FILES['anh_dai_dien']) && $_FILES['anh_dai_dien']['error'] == UPLOAD_ERR_OK) {
        ...
        } elseif (!in_array(strtolower($_FILES['anh_dai_dien']['type']), $allowedTypes)) {
            ...
            if (move_uploaded_file($_FILES['anh_dai_dien']['tmp_name'], $targetPath)) {
                ...
                if (!empty($anh_dai_dien)) {
                    ...
                    if (strpos($anh_dai_dien, 'assets/images/staff/') !== false) {
                        $oldImagePath = '../../client/' . $anh_dai_dien;
                    } else if (strpos($anh_dai_dien, '/') === 0) {
                        $oldImagePath = '../../client' . $anh_dai_dien;
                    } else {
                        $oldImagePath = $uploadDir . basename($anh_dai_dien);
                    }
    ...
    if (empty($passwordError)) {
        ...
        if ($stmt->execute()) {
            ...
            $reloadPage = true; // Flag để JavaScript reload trang
        } else {
...
// Tạo URL avatar với cache busting
$avatarUrl = !empty($nhanVien['anh_dai_dien']) 
    ? '../../client/' . $nhanVien['anh_dai_dien'] . '?v=' . (isset($_GET['t']) ? $_GET['t'] : time())
    : 'https://ui-avatars.com/api/?name=' . urlencode($nhanVien['ho_ten']) . '&background=deb666&color=fff&size=200';
?>
...
<div class="main-content">
        ...
        <div class="row align-items-center">
            <div class="col-auto">
                <div class="avatar-wrapper">
                    <img src="<?php echo h($avatarUrl); ?>" alt="Avatar" class="profile-avatar" id="previewAvatar"
    ...
    <?php if (isset($reloadPage) && $reloadPage): ?>
    <script>
    // Reload trang sau 1.5 giây để hiển thị ảnh mới (tránh cache)
    setTimeout(function() {
        window.location.href = 'index.php?page=profile&t=' + new Date().getTime();
    }, 1500);
    </script>
    <?php endif; ?>
    <?php endif; ?>
...
document.addEventListener('DOMContentLoaded', function() {
        ...
        avatarInput.addEventListener('change', function(e) {
            ...
            if (file) {
                ...
                reader.onload = function(e) {
                    ...
                    previewAvatar.style.transform = 'scale(0.95)';
                    setTimeout(() => {
                        previewAvatar.style.transition = 'transform 0.3s ease';
    ...
    // Force reload ảnh khi có tham số t trong URL (sau khi cập nhật)
