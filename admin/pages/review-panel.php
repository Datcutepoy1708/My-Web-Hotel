...
$room_type_filter = isset($_GET['room_type']) ? intval($_GET['room_type']) : 0;
...
// Tìm kiếm: tên khách hàng, email, số phòng, loại phòng, nội dung đánh giá
if ($search) {
    $where .= " AND (c.full_name LIKE ? OR c.email LIKE ? OR r.comment LIKE ? OR d.room_number LIKE ? OR rt.room_type_name LIKE ?)";
    $searchParam = "%$search%";
...
// Lấy danh sách loại phòng để filter
$roomTypesResult = $mysqli->query("SELECT DISTINCT rt.room_type_id, rt.room_type_name 
                                   FROM room_type rt
                                   INNER JOIN room r ON rt.room_type_id = r.room_type_id
                                   INNER JOIN booking b ON r.room_id = b.room_id
                                   INNER JOIN review rev ON b.booking_id = rev.booking_id
                                   WHERE rev.deleted IS NULL
                                   ORDER BY rt.room_type_name");
$roomTypesList = $roomTypesResult ? $roomTypesResult->fetch_all(MYSQLI_ASSOC) : [];
...
if ($room_type_filter > 0) {
    $statsWhere .= " AND rt.room_type_id = ?";
    $statsParams[] = $room_type_filter;
    $statsTypes .= 'i';
}
...
if ($room_type_filter) $baseUrl .= "&room_type=" . $room_type_filter;
                ...
                <div class="col-md-2">
                    <select class="form-select" name="room_type">
                        ...
                        <?php foreach ($roomTypesList as $rt): ?>
                            <option value="<?php echo $rt['room_type_id']; ?>" <?php echo $room_type_filter == $rt['room_type_id'] ? 'selected' : ''; ?>>
            ...
            <div class="row g-3">
                ...
                <div class="col-md-2">
                    ...
                    <select class="form-select" name="sort_by">
                        ...
                        <option value="room_number" <?php echo $sort_by == 'room_number' ? 'selected' : ''; ?>>Số phòng</option>
                    </select>
                </div>
                <div class="col-md-2">
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <p><strong>Phòng:</strong> <?php echo h($review['room_number']); ?> (<?php echo h($review['room_type_name']); ?>)</p>
