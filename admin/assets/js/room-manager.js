...
function viewRoom(roomId) {
  ...
  if (viewImages) {
    ...
    if (files.length > 0) {
      ...
      // Không sử dụng placeholder external, ẩn ảnh hoặc hiển thị icon
      fallback.style.display = 'none';
      const placeholder = document.createElement('div');
      placeholder.className = 'd-flex align-items-center justify-content-center bg-light rounded';
      placeholder.style.width = '100%';
      placeholder.style.height = '200px';
      placeholder.innerHTML = '<div class="text-center text-muted"><i class="fas fa-image fa-3x mb-2"></i><p class="mb-0">Chưa có ảnh</p></div>';
      fallback.parentNode.replaceChild(placeholder, fallback);
